...
#'
#' Uses OldSeurat::FetchData(training_dge, vars.all = ident.use ) as class labels.
...
train_save_classifier = function(training_dge, results_path, ident.use = "cell_type", genes.use, do.save = F ){
  ...
  # # Get labels
  training_labels = factor( vectorize_preserving_rownames ( OldSeurat::FetchData(training_dge, vars.all = ident.use ) ) )
...
wheel_plot = function( dge, results_path, mlr_mod = NULL, class_labels = NULL, fig_name, 
  ...
  if( !is.null( colour_by ) ){
      cell_positions[[colour_by]] = OldSeurat::FetchData(dge, vars.all = colour_by)[ rownames( predictions ),  ] 
  }
  if( !is.null( facet_by ) ){
    cell_positions[[facet_by]] = OldSeurat::FetchData(dge, vars.all = facet_by )[ rownames( predictions ),  ] 
  }
