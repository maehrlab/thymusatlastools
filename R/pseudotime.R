...
call_monocle_on_seurat = function( dge, results_path, monocle_params, earliest_day = NULL ){
  ...
  pd = Biobase::AnnotatedDataFrame( data = dge@data.info[colnames(raw_dge), ])
  fd = Biobase::AnnotatedDataFrame( data = geneInfo )
  ...
  if(reset_var_genes){
    ...
    cc_genes_all = Reduce(union, cc_genes_macosko)
  ...
  mobj = monocle::reduceDimension(mobj,residualModelFormulaStr=form_str,pseudo_expr=1, reduction_method = reduction_method)
  mobj = monocle::orderCells(mobj, num_paths = num_mature_types)
  
  if ( is.null( earliest_day ) ) {
    earliest_day = monocle::phenoData(mobj)[["eday"]] %>% as.character %>% as.numeric %>% min
  } 
  ...
  eday  = monocle::phenoData(mobj)[["eday"]] %>% as.character %>% as.numeric
  state = monocle::phenoData(mobj)[["State"]] %>% as.character %>% as.numeric
